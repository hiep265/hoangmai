...
""") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=4):
            chatbot_interface = gr.Chatbot(elem_id="chatbot", avatar_images=("static/avt_user.png", "static/avt_bot.png"))
            with gr.Column(elem_id="input-row"):
                msg = gr.Textbox(show_label=False, placeholder="Nhập tin nhắn của bạn ở đây...")
                submit_btn = gr.Button("Gửi", variant="primary")
                buy_now_html = gr.HTML(label="Nút mua", visible=True)
            with gr.Row(elem_id="scroll-buttons"):
